<template>
  ...
  export default {
    ...
    setup() {
      ...
      const { updateDispatchPlace, deleteDispatchPlace } = dispatchPlacesCommand();
      const { getDispatchPlaceSelectedToUpdate } = useDispatchPlaces();
      ...
      return {
        ...
        updateDispatchPlace,
        deleteDispatchPlace,
        getDispatchPlaceSelectedToUpdate,
        ...
        onSubmit: async () => {
          ...
          const id = getDispatchPlaceSelectedToUpdate.value;

          const { errorsUpdate, niceUpdate } = await updateDispatchPlace(
            place.value,
            id
          );
          ...
          if (niceUpdate.value === false) {
            ...
            await store.dispatch("dispatchPlaces/loadDispatchPlaces");
  
            Swal.fire(
              "Guardado",
              "Lugar de despacho actualizado con éxito",
              "success"
        ...
        onDelete: async () => {
          ...
          if (isConfirmed) {
            ...
            const { niceDelete } = await deleteDispatchPlace(
              id
            );
